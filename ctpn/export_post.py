...
def proposal_layer_tf_api(cls_prob_tf, box_pred_tf, im_info_tf):
    rois, _ = proposal_layer(cls_prob_tf, box_pred_tf, im_info_tf, 'TEST', anchor_scales=cfg.ANCHOR_SCALES)
...
def postprocess(cls_prob_tf, box_pred_tf , im_info_tf):
    ...
    boxes=proposal_layer_xxtf_api(cls_prob_tf, box_pred_tf, im_info_tf)
    return boxes
...
def query_ctpn(sess, config, cv2img):
    """Args:
   ...
   Returns:
    ...
    if True:
        ...
        rois, _ = proposal_layer(cls_prob, box_pred, blobs['im_info'],
                                 'TEST', anchor_scales=[16] )
                                 #'TEST', anchor_scales=config.ANCHOR_SCALES)
...
def export():
  ...
  cv2img = cv2.imread("/home/kspook/text-detection-ctpn/data/demo/006.jpg", cv2.IMREAD_COLOR)

  result_boxes=query_ctpn(ctpn_sess, config, cv2img) 
  ...
  jpeg= preprocess_image(image_string)  #预处理,缩放
  ...
  output_tensor_cls_prob,output_tensor_box_pred = tf.import_graph_def\
                           ...
                           #input_map={'Placeholder:0': image_string},
                           input_map={'Placeholder:0': raw_image},
                           return_elements=['Reshape_2:0','rpn_bbox_pred/Reshape_1:0'])

  print('output_tensor_cls_prob, ')
  ...
  prediction_post_signature = (
        tf.saved_model.signature_def_utils.build_signature_def(
           inputs={'images': imageplaceholder_info},
